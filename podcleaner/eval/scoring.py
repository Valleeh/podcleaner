...
# --------------------------------------------------------------------------------------
# bridges to the existing domain models (podcleaner.models)
# --------------------------------------------------------------------------------------


def intervals_from_segments(segments: Iterable[Segment]) -> List[Interval]:
    """Ad intervals from existing :class:`podcleaner.models.Segment` objects."""
    return normalize_intervals(
        [(seg.start, seg.end) for seg in segments if seg.is_ad],
        allow_overlap=True,
        kind="segments",
    )


def intervals_from_transcript(transcript: Transcript) -> List[Interval]:
    """Ad intervals from an existing :class:`podcleaner.models.Transcript`."""
    return intervals_from_segments(transcript.ad_segments)
